...
selector = ((dat['branch-type'] == 2) &
            (dat['squiggle'] > 0.001) &
            (dat['squiggle'] < 2))
dat['selector'].groupby('cell-type').boxplot(column='branch-distance')
dat[selector].groupby('cell-type').boxplot(column='branch-distance')
...
dat[selector].groupby('cell type').apply(np.median)['branch distance (nm)']
dat[selector].groupby('cell type').median()
...
jtj_counts.head()
len(set(jtj['local filename']))
filenames = set(jtj['local filename'])
scale = {f: imageio.imread(f, format='fei').meta['Scan']['PixelHeight']
         for f in filenames}
scales = {}
for f in filenames:
    try:
...
s_scales = pd.Series(scales, name='scales')
s_scales.head()
...
image_info.head()
get_ipython().magic('pinfo pd.join')
np.unique(image_info['number of rows'])
counts = jtjcounts['branch-distance']
counts = jtj_counts['branch-distance']
type(counts)
counts.head()
get_ipython().magic('pinfo image_info.join')
image_info = image_info.join(counts, how='inner')
image_info.head()
get_ipython().magic('pinfo image_info.rename')
get_ipython().magic('pinfo image_info.rename')
image_info = image_info.rename({'branch-distance', 'branch-count'})
image_info = image_info.rename({'branch-distance': 'branch-count'})
image_info.head()
...
image_info.boxplot(column='branches per nm^2', by='cell type')
